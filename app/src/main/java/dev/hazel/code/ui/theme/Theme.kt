package dev.hazel.code.ui.theme
...
/**
 * Hazel runs one palette only: true black. There is no light mode to fall back to, so the
...
private val HazelScheme = darkColorScheme(
    primary = Cyan,
...
private val HazelType = Typography(
...
@Composable
fun HazelTheme(content: @Composable () -> Unit) {
    MaterialTheme(colorScheme = HazelScheme, typography = HazelType, content = content)
}
